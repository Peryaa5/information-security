...
Процесс получения IP-адреса по DHCP состоит из четырёх этапов:

1. **Discovery** — клиент ищет DHCP-сервер.
2. **Offer** — сервер предлагает IP-адрес.
3. **Request** — клиент запрашивает предложенный адрес.
4. **Acknowledgment** — сервер подтверждает выдачу адреса.

В Cisco Packet Tracer этот процесс можно посмотреть в режиме **Simulation**.
